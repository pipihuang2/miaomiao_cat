...
def slice_single_image(input_image_path, output_folder_for_slices, slice_size=(512, 512), preprocess=False, debug=False):
    """
    【辅助函数】
            ...
            for i in range(0, img_height, slice_height):
                for j in range(0, img_width, slice_width):
                    ...
                    if (np.any(np.abs(edge_x) > 60) or np.any(np.abs(edge_y) > 60)):
                        slice_filename = f"{image_name}_{slice_count}.png"
...
def slice_images_in_folder(input_folder, output_folder_for_all_slices, slice_size=(512, 512), preprocess=False, debug=False):
    ...
    image_files = [f for f in os.listdir(input_folder) if os.path.isfile(os.path.join(input_folder, f)) and f.lower().endswith(image_extensions)]
    if not image_files:
        print("输入文件夹中没有找到支持的图像文件。")
        return

    for filename in tqdm(image_files, desc="处理图片文件"):
        input_image_path = os.path.join(input_folder, filename)
        slice_single_image(input_image_path, output_folder_for_all_slices, slice_size, preprocess, debug)

    print(f"\n--- 所有图片处理完成 ---")

if __name__ == "__main__":
    input_folder = r"D:\Project\HYJ_Pic\new_cab\20260104\CamA\20260104"
    base_output_folder = r"D:\Project\HYJ_Pic\new_cab\20260104\CamA\20260104\out"
    slice_size = (256, 256)
    # 设置 preprocess=True 则先进行预处理（二值化+裁剪），否则直接切割原图
    slice_images_in_folder(input_folder, base_output_folder, slice_size, preprocess=False, debug=False)